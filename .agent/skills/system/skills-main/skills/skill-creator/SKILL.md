...
Understanding the triggering mechanism helps design better eval queries. Skills appear in the AI's `available_skills` list with their name + description, and the AI decides whether to consult a skill based on that description. The important thing to know is that AI assistants only consult skills for tasks they can't easily handle on their own — simple, one-step queries like "read this PDF" may not trigger a skill even if the description matches perfectly, because the AI can handle them directly with basic tools. Complex, multi-step, or specialized queries reliably trigger skills when the description matches.
...
### Platforms without subagents (ChatGPT, Claude.ai, basic API usage)

The core workflow is the same (draft → test → review → improve → repeat), but without subagents some mechanics change:
...
**Description optimization**: The optimization loop script requires a CLI tool that can run the AI model programmatically. If your platform doesn't support this, skip it and optimize descriptions manually based on testing.
...
### Platforms with subagents (Claude Code, Kiro, Copilot Workspace)

Full workflow applies — parallel test runs, baseline comparisons, benchmark viewer, and the complete iteration loop.
